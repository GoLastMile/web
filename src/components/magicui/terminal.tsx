...
export function Terminal({
  ...
  // Track visible content height based on activeIndex
  const [visibleHeight, setVisibleHeight] = useState(0)

  // Auto-scroll only when new content would be below the fold
  useEffect(() => {
    if (!scrollRef.current || activeIndex === 0) return

    // Calculate approximate height of visible content (each line ~24px)
    const estimatedContentHeight = activeIndex * 24
    const containerHeight = scrollRef.current.clientHeight

    // Only scroll if content exceeds container
    if (estimatedContentHeight > containerHeight) {
      requestAnimationFrame(() => {
        if (scrollRef.current) {
          scrollRef.current.scrollTo({
            top: estimatedContentHeight - containerHeight + 50,
            behavior: 'smooth'
          })
        }
      })
  ...
  const content = (
    <div
      ref={containerRef}
      className={cn(
        "z-0 h-full w-full max-w-lg border border-outline-variant/30 bg-surface-container",
        className
      )}
    >
      <div className="flex flex-col gap-y-2 border-b border-outline-variant/30 p-4">
